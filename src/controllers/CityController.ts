...
export class CityController extends BaseController {
    ...
    public getAllCities(req: Request, res: Response) {
       ...
       public getAllcityOnbasiOfTrandingPopularDispopular(req: Request, res: Response) {
        const trending = this.sqlService.executeQuery(`select * from ${Tables.cities} where citieStatus = '1' ORDER BY name Asc;`);
        trending.subscribe((result) => {
            const featured = this.sqlService.executeQuery(`select * from ${Tables.cities} where citieStatus = '2' ORDER BY name Asc;`);
